...
namespace SmsghApi.Sdk.Smsgh
{
    ...
    public class ApiAccountProfile
    {
        ...
        /// </summary>
        public ApiAccountProfile(ApiDictionary jso)
        {
            foreach (string key in jso.Keys)
            {
                switch (key.ToLower())
                {
                    ...
                    case "lastaccessed":
                        ...
                        if (jso[key].ToString() != "")
                            _lastAccessed = DateTime.TryParseExact(jso[key].ToString(), "yyyy-dd-MM hh:mm:ss",
                                CultureInfo.InvariantCulture, DateTimeStyles.None, out lastAccessedDate)
                                ? lastAccessedDate
                                : (DateTime?) null;
                        break;
                    case "mobilenumber":
