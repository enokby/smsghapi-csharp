...
namespace SmsghApi.Sdk.Smsgh
{
    ...
    public class ApiSender
    {
        ...
        /// </summary>
        public ApiSender(ApiDictionary jso)
        {
            foreach (string key in jso.Keys)
                switch (key.ToLower())
                {
                    ...
                    case "timedeleted":
                        DateTime td;
                        if (jso[key].ToString() != "")
                            _timeDeleted = DateTime.TryParseExact(jso[key].ToString(), "yyyy-dd-MM hh:mm:ss",
                                CultureInfo.InvariantCulture, DateTimeStyles.None, out td)
                                ? td
                                : (DateTime?)null;
                        break;
                }
